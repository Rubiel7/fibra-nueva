...
function fechaLarga(value: string) {
  ...
  return month && Number.isFinite(y) && Number.isFinite(d) ? `${d} de ${month} de ${y}` : value;
}
function fechaCorta(value: string) {
  const { y, m, d } = parseYMD(value);
  const month = MES_CORTO[m - 1];
  return month && Number.isFinite(y) && Number.isFinite(d) ? `${String(d).padStart(2, "0")}-${month}-${y}` : value;
}
